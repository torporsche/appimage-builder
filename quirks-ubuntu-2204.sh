#!/bin/bash
# Quirks file for Ubuntu 22.04 (ubuntu-latest)
...
quirk_build_msa() {
  # Use system Qt5 instead of /opt/qt59
  add_cmake_options -DQT_RPATH=/usr/lib/x86_64-linux-gnu/
}
...
quirk_build_mcpelauncher_ui() {
  # Use system Qt5 paths instead of /opt/qt59
  add_cmake_options -DCMAKE_INSTALL_RPATH="/usr/lib/x86_64-linux-gnu/:${APP_DIR}/usr/lib/" "-DOPENSSL_SSL_LIBRARY=$PWD/copenssl64/lib64/libssl.so" "-DOPENSSL_CRYPTO_LIBRARY=$PWD/copenssl64/lib64/libcrypto.so" "-DOPENSSL_INCLUDE_DIR=$PWD/copenssl64/include"
}