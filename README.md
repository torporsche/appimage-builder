...
# This step MUST complete successfully before validation can proceed
./build_appimage.sh -t x86_64 -n -j $(nproc) -q quirks-qt6.sh

# CI Troubleshooting: List output directory contents for debugging
...
if [ -d "./output" ]; then
  ...
  # Count AppImages for validation readiness
  ...
  if [ "$appimage_count" -eq 0 ]; then
    echo "❌ CRITICAL: No AppImage files found - validation will fail"
    echo "Check build logs for AppImage creation errors"
    exit 1
  fi
else
  echo "❌ CRITICAL: Output directory ./output does not exist"
  echo "Build step failed to complete successfully"
  echo "Validation cannot proceed without output directory"
  exit 1
fi

# Post-build validation (requires ./output directory and AppImages)
# Early failure detection will catch missing requirements immediately
./validate-appimage.sh
./integration-validation.sh post
```
...
```bash
# CRITICAL WORKFLOW ORDER - build_appimage.sh MUST run before validate-appimage.sh

# Required CI workflow sequence:
# 1. build_appimage.sh → Creates ./output directory and *.AppImage files
# 2. Output directory verification → Ensures build artifacts exist
# 3. validate-appimage.sh → Validates AppImage quality and structure

# The validation script performs early failure detection and requires:
# ✅ ./output directory to exist (created by build script)
# ✅ *.AppImage files in ./output directory (generated by build process)
# ✅ Proper working directory (repository root)

# Common CI failures and IMMEDIATE solutions:
# ❌ "Output directory missing" → build_appimage.sh didn't run or failed
#    Solution: Check build step logs, verify dependencies, retry build
# ❌ "Output directory is empty" → build completed but no AppImage created
#    Solution: Check appimagetool logs, verify AppDir population, check disk space
# ❌ "No AppImage files found" → AppImage creation failed during build
#    Solution: Check linuxdeploy/appimagetool execution, verify AppDir structure

# Debugging workflow for local testing:
./build_appimage.sh -t x86_64 -n -j $(nproc) -q quirks-qt6.sh
echo "Build exit code: $?"  # Should be 0
ls -la ./output/  # Should show *.AppImage and *.zsync files
./validate-appimage.sh  # Should pass with early checks and full validation

# The validation script now includes comprehensive early failure detection
# that will immediately identify and report CI workflow ordering issues
```
